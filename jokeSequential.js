...
var sequentialDigits = function (low, high) {
  ...
  const result = [];
  for (let i = 0; i < sequentialDigitsList.length; i++) {
    if (sequentialDigitsList[i] > high) break;
    if (sequentialDigitsList[i] >= low) {
      result.push(sequentialDigitsList[i]);
    }
  }
  return result;
};
